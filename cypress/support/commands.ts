...
function enterInsuredPageDetails(
  country: string,
  insured: string,
  ...
  if (!isEmpty(country)) {
    cy.xpath(
      '//*[contains(@data-params,"Primary Country")]//*[text()="Choose"]'
    ).click();
  ...
  if (!isEmpty(insured)) {
    cy.xpath(
      '//*[contains(@data-params,"Primary Insured")]//*[text()="Choose"]'
    ).click();
  ...
  if (numberOfMonths === 0) {
    ...
    cy.get("body").then((body) => {
      if (body.find('[aria-label="Day of the month"]').length > 0) {
        cy.xpath('//input[@aria-label="Day of the month"]')
          .type("0")
          .tab({ shift: true });
      } else {
